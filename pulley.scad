...
//function from https://en.wikibooks.org/wiki/OpenSCAD_User_Manual/undersized_circular_objects
...
//	translate ([0,0, pulley_b_ht + pulley_t_ht + retainer_ht ]) rotate ([0,180,0])
	...
	difference()
	 {	 
        ...
        //text
        translate([0,pulley_OD/2 + retainer_ht + text_offset,pulley_t_ht + retainer_ht]) {
            scale([1,1,-1]) {
                linear_extrude(2) {
        ...
        //holes for hub
        if (hub_holes) {
            for (rotation=[0:60:300]) {
                rotate([0,0,rotation]) {
                    translate([(1.875*25.4)/2,0,-50]) {
                        cylinder(d=0.2*25.4 + 1,h=100,$fn=50);
                    }
                }
            }
        }
